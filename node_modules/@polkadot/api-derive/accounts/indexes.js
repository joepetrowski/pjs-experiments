...
function indexes(api) {
  return (0, _util.memo)(() => api.query.indices.nextEnumSet().pipe( // use the nextEnumSet (which is a counter of the number of sets) to construct
  ...
  (0, _operators.switchMap)(next => api.query.indices.enumSet.multi([...Array(next.toNumber() + 1).keys()])), (0, _operators.map)(all => all.reduce((result, list, outerIndex) => {
    (list || []).forEach((accountId, innerIndex) => {
      ...
      result[accountId.toString()] = (0, _types.createType)('AccountIndex', index);
    });
    return result;
  }, {}))));
}