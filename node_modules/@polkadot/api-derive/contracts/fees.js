...
function queryV1(api) {
  return api.queryMulti([api.query.contract.callBaseFee, api.query.contract.contractFee, api.query.contract.createBaseFee, api.query.contract.creationFee, api.query.contract.transactionBaseFee, api.query.contract.transactionByteFee, api.query.contract.transferFee]).pipe((0, _operators.map)((_ref2) => {
    let [callBaseFee, contractFee, createBaseFee, creationFee, transactionBaseFee, transactionByteFee, transferFee] = _ref2;
    return (// We've done this on purpose, i.e. so we can  just copy the name/order from the parse above and
      // see gaps, in this case those are filled with `ZERO`
      parseResult([callBaseFee, contractFee, createBaseFee, creationFee, ZERO, ZERO, ZERO, transactionBaseFee, transactionByteFee, transferFee])
    );
  }));
} // query via query (early v2, non-current, support to be dropped])


function queryQuery(api) {
  const queryBase = api.query.contracts || api.query.contract;
  return api.queryMulti([queryBase.callBaseFee, queryBase.contractFee, queryBase.createBaseFee, queryBase.creationFee, queryBase.rentByteFee, queryBase.rentDepositOffset, queryBase.tombstoneDeposit, queryBase.transactionBaseFee, queryBase.transactionByteFee, queryBase.transferFee]);
...
function fees(api) {
  return (0, _util.memo)(() => {
    ...
    return (api.consts.contracts ? queryConstants(api) : queryQuery(api)).pipe((0, _operators.map)((_ref3) => {
      let [callBaseFee, contractFee, createBaseFee, creationFee, rentByteFee, rentDepositOffset, tombstoneDeposit, transactionBaseFee, transactionByteFee, transferFee] = _ref3;
